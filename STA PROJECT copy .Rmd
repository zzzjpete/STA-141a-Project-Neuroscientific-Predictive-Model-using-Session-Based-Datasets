...
# Abstract
In this project, our journey started with an exploratory data analysis, where we delved into the features across multiple sessions, including the number of neurons, trials, mouse name, brain area, stimuli conditions, and success rate. This set the stage for our next step, which was an in-depth exploration of neural activities within the fifth session. This involved calculating the peak firing rate for each neuron and visualizing these rates using a histogram. 

Subsequently, we turned our focus to the failure rates, examining their average values across all sessions. Our journey through data exploration did not stop there. We moved on to investigating changes across trials, specifically analyzing shifts in the "feedback_type" variable. 

This understanding paved the way for us to identify homogeneity and heterogeneity across sessions and mice, delving deeper into the outcomes of trials, i.e., successes and failures. This investigation revealed shared patterns across sessions, which we harnessed to create a sub-data frame from the entire dataset. The rationale behind this was to optimize our prediction model's performance. 

For our sub-data frame, we carefully selected five variables: "contrast_left," "contrast_right," 'spks_mean,' "spks_sd," and "total_neurons." Armed with this consolidated data, we performed K-means clustering and Principal Component Analysis (PCA), presenting us with more understandable and manageable information. 

Finally, we moved onto model creation, splitting our sub-data frame into training and testing data, and fitting a logistic regression model. We tested the model's performance using two distinct test sets, drawing insightful conclusions and interpretations from the results. This comprehensive exploration and analysis journey led us to a better understanding of the data and the factors driving it.
